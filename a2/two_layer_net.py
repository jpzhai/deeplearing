...
def nn_forward_pass(params, X):
    ...
    - params: a dictionary of PyTorch Tensor that store the weights of a model.
      It should have following keys with shape
          ...
          W2: Second layer weights; has shape (H, C)
          b2: Second layer biases; has shape (C,)
    - X: Input data of shape (N, D). Each X[i] is a training sample.一共有N个训练图像，X[i]是第i个训练图像

    Returns a tuple of:
    - scores: Tensor of shape (N, C) giving the classification scores for X   评分函数
    - hidden: Tensor of shape (N, H) giving the hidden layer representation   隐藏层
      for each input value (after the ReLU激活函数).
    """
...
def nn_get_search_params():
  ...
  regularization = [5e-5, 9e-5,  1e0, 7e-4]
      ...
      for hs in hidden_sizes:
          for reg in regularization_strengths:
              for lrd in learning_rate_decays:
                  net = TwoLayerNet(3*32*32, hs, num_classes, dtype = data_dict['X_train'].dtype, device=data_dict['X_train'].device)
