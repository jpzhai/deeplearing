...
def compute_distances_two_loops(x_train, x_test):
  ...
  x_train_f = x_train.clone().reshape(num_train,-1)
  x_test_f = x_test.clone().reshape(num_test,-1)
  for j in range(num_test):
      for i in range(num_train):
        dists[i,j] = torch.sqrt(torch.sum((x_test_f[j]-x_train_f[i])**2))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  x_test_f = x_test.clone().reshape(num_test,-1)
  for i in range(num_train):
    dists[i,:] = torch.sqrt(torch.sum((x_train_f[i]-x_test_f)**2,dim=1))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  x_train_f = x_train.clone().reshape(num_train,-1)
  x_test_f = x_test.clone().reshape(num_test,-1)
  dists = torch.sqrt(torch.sum(x_test_f**2,dim=1)+torch.sum(x_train_f**2,dim=1).reshape(-1,1)-2*torch.mm(x_train_f,x_test_f.t()))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  for i in range(num_test):
     ...
     y_pred[i],_ = torch.mode(k_nearest)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  y_train_folds = []
  ##############################################################################
  # TODO: Split（分裂） the training data and images into folds. After splitting,   #
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
       ...
       x_val_train = x_train_folds[:]
       y_val_train = y_train_folds[:]
       del x_val_train[i]
       del y_val_train[i]
       x_train_c = x_val_train[len(x_val_train)-1]
       y_train_c = y_val_train[len(y_val_train)-1]
       for i in range(len(x_val_train)-1):
       	  x_train_c = torch.cat((x_train_c,x_val_train[i]),dim=0)
       	  y_train_c = torch.cat((y_train_c,y_val_train[i]),dim=0)
       classifier= KnnClassifier(x_train_c,y_train_c)
       p_accuracy = classifier.check_accuracy(x_val_test,y_val_test,k=k,quiet=True)
       accuracy.append(p_accuracy)
